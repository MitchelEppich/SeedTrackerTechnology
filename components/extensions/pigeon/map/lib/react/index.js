...
function wikimedia(x, y, z) {
  ...
  return (
    "http://services.arcgisonline.com/ArcGIS/rest/services/Canvas/World_Dark_Gray_Base/MapServer/tile/" +
    z +
    "/" +
    y +
    "/" +
...
var requestAnimationFrame = hasWindow
  ? window.requestAnimationFrame || window.setTimeout
  : function(callback) {
      return callback();
    };
var cancelAnimationFrame = hasWindow
  ...
  function Map(props) {
      ...
      if (
        ...
        if (_this._isAnimating) {
          ...
          _this._centerStart = _this.limitCenterAtZoom(
            [_this._lastCenter[0], _this._lastCenter[1]],
            _this._lastZoom
          );
        ...
        if (zoomAround) {
          ...
          _this._centerTarget = _this.calculateZoomCenter(
            _this._lastCenter,
            zoomAround,
            _this._lastZoom,
            zoom
        ...
        if (zoomAround) {
          var _center = _this.calculateZoomCenter(
            _this._lastCenter,
            zoomAround,
            _this._lastZoom,
            zoom
    ...
    _this.animationStep = function(timestamp) {
      ...
      if (_this._zoomAround) {
        var centerStep = _this.calculateZoomCenter(
          _this._centerStart,
          _this._zoomAround,
          _this._zoomStart,
          zoomStep
        ...
        var _centerStep = [
          _this._centerStart[0] +
            (_this._centerTarget[0] - _this._centerStart[0]) * percentage,
          _this._centerStart[1] +
            (_this._centerTarget[1] - _this._centerStart[1]) * percentage
        ];
    ...
    _this.limitCenterAtZoom = function(center, zoom) {
      ...
      return [
        Math.max(
          Math.min(
            isNaN(center[0]) ? _this.state.center[0] : center[0],
            minMax[1]
          ),
          minMax[0]
        ),
        Math.max(
          Math.min(
            isNaN(center[1]) ? _this.state.center[1] : center[1],
            minMax[3]
          ),
          minMax[2]
        )
    ...
    _this.setCenterZoom = function(center, zoom) {
      ...
      var maybeCenter = _this.props.center
        ? _this.props.center
        : _this._lastCenter;
      if (
        animationEnded ||
      ...
      if (event.touches.length === 1) {
        ...
        if (_this.coordsInside(pixel)) {
          ...
          if (!_this.props.twoFingerDrag) {
            ...
            if (
              _this._lastTap &&
              performanceNow() - _this._lastTap < DOUBLE_CLICK_DELAY
            ) {
              ...
              _this.setCenterZoomTarget(
                ...
                Math.max(
                  _this.props.minZoom,
                  Math.min(_this.state.zoom + 1, _this.props.maxZoom)
                ),
                false,
                latLngNow
              );
        ...
        _this._touchStartDistance = Math.sqrt(
      ...
      if (event.touches.length === 1 && _this._touchStartPixel) {
        ...
        var zoomDelta =
          Math.max(
            ...
            Math.min(
              _this.props.maxZoom,
              zoom + Math.log2(distance / _this._touchStartDistance)
            )
          ) - zoom;
      ...
      if (_this._touchStartPixel) {
        ...
        if (event.touches.length === 0) {
          if (twoFingerDrag) {
            ...
            var newTouchPixel = getMousePixel(
              _this._containerRef,
            ...
            if (
              ...
              if (
                !_this._secondTouchEnd ||
                performanceNow() - _this._secondTouchEnd >
          ...
          if (zoomSnap) {
            ...
            var latLng = _this._touchStartMidPoint
              ? _this.pixelToLatLng(_this._touchStartMidPoint)
              : _this.state.center;
            ...
            if (
              twoFingerDrag &&
              Math.round(_this.state.zoom) ===
                Math.round(_this.state.zoom + zoomDelta)
            ) {
              zoomTarget = Math.round(_this.state.zoom);
            } else {
              zoomTarget =
                zoomDelta > 0
                  ? Math.ceil(_this.state.zoom)
                  : Math.floor(_this.state.zoom);
            }
    ...
    _this.handleMouseDown = function(event) {
      ...
      if (
        event.button === 0 &&
        (!event.target ||
          !(0, _parentHasClass2.default)(event.target, "pigeon-drag-block")) &&
        ...
        if (
          _this._lastClick &&
          performanceNow() - _this._lastClick < DOUBLE_CLICK_DELAY
        ) {
    ...
    _this.trackMoveEvents = function(coords) {
      ...
      if (
        ...
        timestamp - _this._moveEvents[_this._moveEvents.length - 1].timestamp >
          40
      ) {
      ...
      if (lastEvent && animate) {
        ...
        if (distance > MIN_DRAG_FOR_THROW) {
          ...
          _this.setCenterZoomTarget(
            ...
            false,
            null,
            (DIAGONAL_THROW_TIME * distance) / diagonal
          );
        }
      }
      ...
      if (!metaWheelZoom || event.metaKey) {
        ...
        if (!zoomSnap && _this._zoomTarget) {
          ...
          if (animate) {
            ...
            if (
              !_this._lastWheel ||
              performanceNow() - _this._lastWheel > ANIMATION_TIME
            ) {
    ...
    _this.showWarning = function(warningType) {
      ...
      _this._warningClearTimeout = window.setTimeout(
        _this.clearWarning,
    ...
    _this.syncToProps = (0, _debounce2.default)(
      _this.syncToProps,
      DEBOUNCE_DELAY
    );
    ...
    _this._lastCenter = props.defaultCenter
      ? props.defaultCenter
      : props.center;
  ...
  _createClass(Map, [
    ...
    },
    {
      ...
      value: function(nextProps) {
        ...
        var maybeCenter = nextProps.center
          ? nextProps.center
          : this.state.center;
        var maybeZoom = nextProps.zoom ? nextProps.zoom : this.state.zoom;
        if (
          Math.abs(maybeZoom - this.state.zoom) > 0.001 ||
    ...
    },
    {
      ...
      value: function() {
        ...
        for (var _x12 = xMin; _x12 <= xMax; _x12++) {
          for (var _y = yMin; _y <= yMax; _y++) {
            tiles.push({
              key: _x12 + "-" + _y + "-" + roundedZoom,
              url: mapUrl(_x12, _y, roundedZoom),
              left: (_x12 - tileMinX) * 256,
              top: (_y - tileMinY) * 256,
              width: 256,
              height: 256,
              active: true
          ...
          _infact.React.createElement(
            "div",
            {
              style: {
                ...
                transform:
                  "translate(" +
                  -((tileCenterX - tileMinX) * 256 - scaleWidth / 2) +
                  "px, " +
                  -((tileCenterY - tileMinY) * 256 - scaleHeight / 2) +
                  "px)"
              }
            },
    ...
    },
    {
      ...
      value: function() {
        ...
        childrenWithProps = _infact.React.Children.map(
          this.props.children,
          function(child) {
            if (!child) {
              return null;
...
Map.propTypes = {
  ...
  limitBounds: _propTypes2.default.oneOf(["center", "edge"])
};
Map.defaultProps = {
  animate: true,
  metaWheelZoom: false,
